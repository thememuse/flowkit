...
def generate_claude(skills):
    ...
    claude_fk_dir = CLAUDE_COMMANDS_DIR / "fk"
    claude_fk_dir.mkdir(parents=True, exist_ok=True)
    count = 0
    for skill in skills:
        name = skill["name"]
        desc = skill["description"]
        dest = claude_fk_dir / f"{name}.md"
        content = (
            f"{desc}\n"
...
def do_clean():
    ...
    if CLAUDE_COMMANDS_DIR.exists():
        # Legacy (pre-Windows-safe) filenames used fk:<name>.md at top-level.
        for f in CLAUDE_COMMANDS_DIR.glob("fk:*.md"):
            f.unlink()
            removed.append(str(f))

        # Current namespace layout: .claude/commands/fk/<name>.md
        claude_fk_dir = CLAUDE_COMMANDS_DIR / "fk"
        if claude_fk_dir.exists():
            for f in claude_fk_dir.glob("*.md"):
                f.unlink()
                removed.append(str(f))
            try:
                claude_fk_dir.rmdir()
